 ...
 */
    ...
    $this->out(__('Checking Aco node: %s', $path), 1, Shell::VERBOSE);
    if (!empty($alias)) $this->out(__('--Alias: %s', $alias), 1, Shell::VERBOSE);
    if (!empty($parentId)) $this->out(__('--ParentId: %s', $parentId), 1, Shell::VERBOSE);
    $node = $this->Aco->node($path);
      ...
      $_node = $this->Aco->node($path);

      if (!isset($_node[0]) or !isset($_node[0]['Aco']['alias'])) {
        $this->out(__('Failed second check.'), 1, Shell::VERBOSE);
        $this->out(__('!!!'));
        exit;
      }
		} else {
